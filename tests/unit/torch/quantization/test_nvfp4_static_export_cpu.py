...
"""CPU round-trip tests for the NVFP4 static-quantizer export path.

These tests target the failure mode seen on Nemotron-style routed-expert weight
matrices: most blocks have tiny per-block amax (1e-7 ish) while a few outlier
blocks carry the global tensor max (1e-1 ish). MSE calibration produces a
per-block ``_amax`` that may be either smaller (outlier-clipping) or larger
(less-clipping) than the actual block max, while ``_global_amax`` is held to the
max-calibrated tensor max. The static export path
(:func:`NVFP4QTensor.get_weights_scaling_factor_from_quantizer` -> static branch)
must produce a finite FP8 ``weight_scale`` and a packed FP4 ``weight`` whose
round-trip dequantization is finite and bounded for *every* such block,
including the underflow and overflow extremes. Any NaN/Inf or unbounded value
on the export side propagates straight into vLLM dequantization, where it
manifests as NaN logits and ``<unk>`` outputs.
"""
