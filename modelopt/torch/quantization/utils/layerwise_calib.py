...
def _write_manifest(checkpoint_dir: str, last_completed_layer: int, num_layers: int) -> None:
    """Atomically write manifest.json — the single commit point per iteration."""
    path = os.path.join(checkpoint_dir, "manifest.json")
    tmp = path + ".tmp"
...
class _CheckpointState:
    ...
    def setup_resume(self, layers: nn.ModuleList) -> list | None:
        ...
        skip mode can produce correctly shaped dummy outputs. The activations
        are read from ``layer_{K-1:04d}/next_inputs.pt``.
        """
        ...
        next_inputs_path = os.path.join(
            _layer_dir(self.checkpoint_dir, last_ckpt), "next_inputs.pt"
        )
        if not os.path.isfile(next_inputs_path):
            raise FileNotFoundError(
                f"Cannot resume: next_inputs.pt missing for layer {last_ckpt}: {next_inputs_path}"
            )
        # weights_only=False is safe: file is internally generated by _save_layer, not user-supplied
